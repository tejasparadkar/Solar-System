...
void planetMotion(int xrad, int yrad, int midx, int midy, int x[60], int y[60]) { int i, j = 0;

/* positions of planets in their corresponding orbits */ for (i = 360; i > 0; i = i - 6) {
...
initgraph(&gdriver, &gmode, "C:/TURBOC3/BGI"); err = graphresult();
...
/* positions of planets on their corresponding orbits */ for (i = 0; i < 9; i++) {
...
jupiter in fifth orbit */ setcolor(BROWN); setfillstyle(SOLID_FILL, BROWN); pieslice(x[4][pos[4]], y[4][pos[4]], 0, 360, planet[4]);
